	            ...
	            @include('includes.messageinnerboard')
	            	...
	            	@if(isset($_GET['sn']) || isset($_GET['dfrom']) || isset($_GET['dend']) || isset($_GET['leadby']))
		            	<div class="adminFiltersQuery">
		                    ...
		                    <a class="adminFiltersQuery__link" href="{{ url('messages-read') }}">Delete</a>
		                </div>
		            @endif

	                @if(count($data['readMessage']) > 0)
		                <div class="messagecontent-wr">
		                  	<form id="frmGestorSolicitudes" name="frmGestorSolicitudes" action="{{ url('messages-status') }}" method="post">
					            ...
					            <ul class="adminHomeSol">
					               @foreach($data['readMessage'] as $read)
			                        <li class="app-vendor-message-{{$read->id}} adminHomeSol__item pure-g @if(!$read->read_status) adminHomeSol__item--new @endif">
			                              <div class="pure-u-7-10">
			                                 <div class="pure-g">
			                                    <div class="adminHomeSol__checkAvatar pure-u-2-12">
			                                       <div class="adminHomeSol__check fleft">
			                                          <div class="icheckbox_minimal single_icheckbox_minimal" style="position: relative;"><input class="app-solicitudes-check dnone" type="checkbox" name="messageids[]" id="messageids{{ $read->id }}" value="{{ $read->id }}" style="position: absolute; top: -20%; left: -20%; display: block; width: 140%; height: 140%; margin: 0px; padding: 0px; background: rgb(255, 255, 255); border: 0px; opacity: 0;"><ins class="iCheck-helper" style="position: absolute; top: -20%; left: -20%; display: block; width: 140%; height: 140%; margin: 0px; padding: 0px; background: rgb(255, 255, 255); border: 0px; opacity: 0;"></ins>
			                                          </div>
			                                       </div>
			                                       <div class="adminHomeSol__avatar app-link">
			                                          @if($read->user['profile_image'])
			                                              <figure>
			                                                  <img class="avatar-thumb" src="{{  url('storage/USER_') }}{{ $read->user['id'] }}/{{ $read->user['profile_image'] }}" alt="Emily" width="60" height="60">
			                                              </figure>
			                                            @else
			                                              <div class="avatar-alias size-avatar-medium ">
			                                                 <svg class="avatar-generic" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" preserveAspectRatio="xMidYMin slice">
			                                                    ...
			                                                    <text transform="translate(100,130)" y="0">
			                                                       <tspan font-size="90" class="" fill="rgba(0,0,0,0.3)" text-anchor="middle">{{ ucfirst($read->user['name'][0]) }}</tspan>
			                                                    </text>
			                                                 </svg>
			                                              </div>
			                                          @endif
			                              ...
			                              <div class="pure-u-3-10">
			                                <div clas="pure-g">
			                                   <div class="adminHomeSol__info cmn_date_wrp pure-u-1-2">
			                                      ...
			                                      <time class="adminHomeSol__info-number"> {{ date('d', strtotime($read->created_at)) }} {{ date('M', strtotime($read->created_at)) }} <span class="adminHomeSol__info-extra">{{ date('Y', strtotime($read->created_at)) }}</span></time>
			                                   </div>
			                                </div>
			                             </div>
			                         </li>
				        ...
				        <div class="mb20">
				            {!! $data['readMessage']->appends(Input::except('page'))->render() !!}
				        </div>
				    @else 
