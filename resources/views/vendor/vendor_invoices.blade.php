	...
	<div class="wrapper">
		<div class="pure-g">
			<div class="pure-u-2-7">
				<div class="mr40">
					<nav class="adminAside billing_list">
						<a class="adminAside__item active" href="{{url('invoices')}}">
			...
			<div class="pure-u-5-7">
				<h1 class="adminTitle">Invoices <span class="adminTitle__counter">({{ count($data['vendorInvc']) }})</span></h1>
				<div class="adminAlert">
					<p class="adminAlert__title billing_alert_text">Check your invoices for services on My Health Squad.</p>
					<p>By clicking on each invoice you can see all the information about the services you received and get a printable copy.</p>
				</div>
				@php
					$curYear = date('Y');
					$subs_date = @$data['vendorInvc'][0]->subscription_date;
					if($subs_date) {
						$subsYear = date_format(date_create($subs_date),'Y');
					} else {
						$subsYear = date('Y');
					}
				@endphp
				<div class="app-ui-dropdown adminFiltersBox__select adminFiltersBox__select--blood billing_year_wrp">
					<span class="billing_year_text"> {{ $curYear }} </span> <i class="icon icon-arrow-down"></i>
					<ul class="app-ui-dropdown-layer adminFiltersBox__dropdown billing_dropdown" style="display:none;">
						@for($ndt = $curYear; $ndt >= $subsYear; $ndt--)
						<li><a data-iditem="{{ $ndt }}" class="app-filter-bills @if($curYear == $ndt) active @endif">{{ $ndt }}</a></li>
						@endfor
					</ul>
				</div>
				<table class="pure-table mt20 mb20 billing_table" cellpadding="0" cellspacing="0" width="100%">
					<thead>
						<tr>
							<td align="right" width="25%"> <p class="m0">Date</p> </td>
							<td align="right" width="25%"> <p class="m0">Bill No.</p> </td>
							<td align="right" width="25%"> <p class="m0">Amount</p> </td>
							<td align="right" width="25%">&nbsp;</td>
						</tr>
					</thead>
					<tbody>
						@foreach($data['vendorInvc'] as $inv)
						<tr>
							<td align="right" valign="center">{{ date_format(date_create($inv->subscription_date),'d/m/Y') }}</td>
							<td align="right" valign="center"> {{ $inv->invoice_id }} </td>
							<td align="right" valign="center">C$ {{ $inv->subscription_amount }}</td>
							<td align="right" valign="center">
								<div class="overflow text-center">
									<!-- <a class="adminDownload regular" target="frmSubmit" href="javascript:;"> -->
									<a class="adminDownload regular" target="_blank" href="{{ url('download-invoice').'/'.encrypt($inv->id) }}">
										<i class="svgIcon svgIcon__bill adminAside__icon adminAside__icon--blood">
											<svg viewBox="0 0 16 20">
												<path d="M14.196 5.729L9.64 1.8H1.804v16.4h12.392V5.729zm.5 13.471H1.304a.5.5 0 0 1-.5-.5V1.3a.5.5 0 0 1 .5-.5h8.522a.5.5 0 0 1 .327.121l4.87 4.2a.5.5 0 0 1 .173.379v13.2a.5.5 0 0 1-.5.5zM10.326 5h4.37a.5.5 0 0 1 0 1h-4.87a.5.5 0 0 1-.5-.5V1.3a.5.5 0 1 1 1 0V5zM3.24 17.4V6.8h9.522v10.6H3.239zm8.522-1V7.8H4.239v8.6h7.522zm-8.022-6.2a.5.5 0 0 1 0-1h8.522a.5.5 0 0 1 0 1H3.739zm0 2.4a.5.5 0 1 1 0-1h8.522a.5.5 0 0 1 0 1H3.739zm0 2.4a.5.5 0 1 1 0-1h8.522a.5.5 0 0 1 0 1H3.739zm2.544-7.7a.5.5 0 1 1 1 0v9.6a.5.5 0 1 1-1 0V7.3z" fill-rule="nonzero"></path>
											</svg>
										</i> Print invoice
...
$(document).ready(function(){
	$('.adminFiltersBox__select').click(function(){
		$('.billing_dropdown').toggle();
	});
	$('.app-filter-bills').click(function() {
		var selYear = $(this).data('iditem');
		$(".billing_year_text").html(selYear);
	});
});
</script>
@endsection