			...
			<div class="pure-u-2-7">
				<div class="mr40">
					<nav class="adminAside billing_list">
						...
						<a class="adminAside__item {{ request()->segment(2) == 'add-blogs' ? 'active' : 'no'  }}" href="{{url('add-blogs')}}"> Add New Blog </a>
					</nav>
				</div>
			</div>
			<div class="pure-u-5-7">
				<h1 class="adminTitle">Edit Your Blog</h1>
				<div class="adminAlert adminAlert--flex">
                    <i class="icon-vendors-admin-alerts icon-vendors-admin-alerts-reviews"></i>
                    <div>
                       <p class="adminAlert__title review_count"> Share your blog... </p>
                    </div>
                </div>
                <div class="delete-wi-wr">
                	<a onclick="return confirm('Are you sure ?');" href="{{url('delete-wedding-ideas').'/'.$data['weddingPost'][0]->id}}">Delete Post</a>
                </div>
                ...
                <form class="pure-form mb20" id="reviewrequestSend" name="reviewrequestSend" method="post" action="{{url('save-wedding-ideas')}}">
                    ...
                    <div class="border p30 review_reci_frm">
                       <div class="pb5 reviewCollector__addUsers collector_wrp">
                          	<div class="mb10 app-review-collector-templates-user-form">
                              	<div class="pure-u-1 drop-wrapper">
                              	  	...
                              	  	<input type="hidden" name="idx" value="{{ $data['weddingPost'][0]->id }}">
                                  	<input class="wi-input" placeholder="Your Idea" name="widdingIdeastitle" type="text" value="{{ $data['weddingPost'][0]->post_title }}" autocomplete="off">
                          	...
                          	<div class="mb10 app-review-collector-templates-user-form">
	                      		<div class="pure-u-1 drop-wrapper">
	                              	...
	                              	<input class="wi-input" placeholder="Your Thinking.." name="widdingIdeasSubtitle" type="text" value="{{ $data['weddingPost'][0]->post_sub_title }}" autocomplete="off">
				                            ...
				                            <select class="pure-u-1" name="WIparentCatgories" id="WIparentCatgories">
												...
												@if(count($data['parentCategory']) > 0)
													@foreach($data['parentCategory'] as $parentCat)
														<option value="{{ $parentCat->id }}" @if($parentCat->id == $data['weddingPost'][0]->parent_cat_id) selected @endif>{{ $parentCat->title }}</option>
													@endforeach
												@endif
				                            </select>
				                        </div>
				                    ...
				                    <div class="mt10">
				                        <div class="select-fake pure-u-1">
				                            <select class="pure-u-1" name="WIparentSubCatgories" id="WIparentSubCatgories">
			                                    <option value="">-- Select Category --</option>
			                                    <option selected value="{{ $data['weddingPost'][0]->subCategory->id }}">{{ $data['weddingPost'][0]->subCategory->title }}</option>
				                            </select>
				                        </div>
				                    </div>
				                </div>
                       ...
                       <div class="mt30 relative collector_wrp">
                      		<input class="btnFlat btnFlat--primary mt15 app-submit-review-request send_btn" type="submit" name="submit" value="Update Your Idea">
                       </div>
                    </div>
                 </form>
			</div>
...
$(document).ready(function()
{
	...
	var content = {!! json_encode( $data['weddingPost'][0]->content ) !!};
