              ...
              @include('includes.messageinnerboard')
                    ...
                    @if(isset($_GET['sn']) || isset($_GET['dfrom']) || isset($_GET['dend']) || isset($_GET['leadby']))
                      <div class="adminFiltersQuery">
                            ...
                            <a class="adminFiltersQuery__link" href="{{ url('messages-unread') }}">Delete</a>
                      </div>
                    @endif

                    @if(count($data['unreadMessage']) > 0)
                                                      ...
                                                      <div class="avatar-alias size-avatar-medium ">
                                                         <svg class="avatar-generic" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" preserveAspectRatio="xMidYMin slice">
                                                            ...
                                                            <text transform="translate(100,130)" y="0">
                                                               <tspan font-size="90" class="" fill="rgba(0,0,0,0.3)" text-anchor="middle">{{ ucfirst($unread->user['name'][0]) }}</tspan>
                                                            </text>
                                                         </svg>
                                                      </div>
                                                  @endif
                                      ...
                                      <div class="pure-u-3-10">
                                        <div clas="pure-g">
                                           <div class="adminHomeSol__info cmn_date_wrp pure-u-1-2">
                                              ...
                                              <time class="adminHomeSol__info-number"> {{ date('d', strtotime($unread->created_at)) }} {{ date('M', strtotime($unread->created_at)) }} <span class="adminHomeSol__info-extra">{{ date('Y', strtotime($unread->created_at)) }}</span></time>
                                           </div>
                                        </div>
                                     </div>
                                    </li>
                        ...
                        <div class="mb20">
                            {!! $data['unreadMessage']->appends(Input::except('page'))->render() !!}
                        </div>
                    @else 
