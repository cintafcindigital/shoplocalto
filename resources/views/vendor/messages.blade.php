                 ...
                 @include('includes.messageinnerboard')
                  ...
                  @if(isset($_GET['sn']) || isset($_GET['dfrom']) || isset($_GET['dend']) || isset($_GET['leadby']))
                    <div class="adminFiltersQuery">
                          ...
                          <a class="adminFiltersQuery__link" href="{{ url('messages') }}">Delete</a>
                      </div>
                  @endif

                  @if(count($data['inboxMessage']) > 0)
                     ...
                     <div class="messagecontent-wr">
                        <form id="frmGestorSolicitudes" name="frmGestorSolicitudes" action="{{ url('messages-status') }}" method="post">
                            ...
                            <ul class="adminHomeSol">
                              ...
                              @foreach($data['inboxMessage'] as $inbox)

                                <li class="app-vendor-message-{{$inbox->id}} adminHomeSol__item pure-g @if(!$inbox->read_status) adminHomeSol__item--new @endif">
                                  <div class="pure-u-7-10">
                                     <div class="pure-g">
                                        <div class="adminHomeSol__checkAvatar pure-u-2-12">
                                           <div class="adminHomeSol__check fleft">
                                              <div class="icheckbox_minimal single_icheckbox_minimal" style="position: relative;"><input class="app-solicitudes-check dnone" type="checkbox" name="messageids[]" id="messageids{{ $inbox->id }}" value="{{ $inbox->id }}" style="position: absolute; top: -20%; left: -20%; display: block; width: 140%; height: 140%; margin: 0px; padding: 0px; background: rgb(255, 255, 255); border: 0px; opacity: 0;"><ins class="iCheck-helper" style="position: absolute; top: -20%; left: -20%; display: block; width: 140%; height: 140%; margin: 0px; padding: 0px; background: rgb(255, 255, 255); border: 0px; opacity: 0;"></ins>
                                              </div>
                                           </div>
                                            <div class="adminHomeSol__avatar app-link">
                                                <a href="{{ url('message-details') }}/{{ $inbox->id }}">
                                                @if(isset($inbox->user['profile_image']) && 1 == 2)
                                                  <figure>
                                                      <img class="avatar-thumb" src="{{  url('storage/USER_') }}{{ $inbox->user['id'] }}/{{ $inbox->user['profile_image'] }}" alt="Emily" width="60" height="60">
                                                  </figure>
                                                @else
                                                  <div class="avatar-alias size-avatar-medium ">
                                                     <svg class="avatar-generic" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" preserveAspectRatio="xMidYMin slice">
                                                        ...
                                                        <text transform="translate(100,130)" y="0">
                                                           <tspan font-size="90" class="" fill="rgba(0,0,0,0.3)" text-anchor="middle">{{ ucfirst(substr($inbox->name,0,1)) }}</tspan>
                                                        </text>
                                                     </svg>
                                                  </div>
                                                @endif
                                  ...
                                  <div class="pure-u-3-10">
                                    <div clas="pure-g">
                                       <div class="adminHomeSol__info cmn_date_wrp pure-u-1-2">
                                          ...
                                          <time class="adminHomeSol__info-number"> {{ date('d', strtotime($inbox->created_at)) }} {{ date('M', strtotime($inbox->created_at)) }} <span class="adminHomeSol__info-extra">{{ date('Y', strtotime($inbox->created_at)) }}</span></time>
                                       </div>
                                    </div>
                                 </div>
                                </li>
                      ...
                      <div class="mb20">
                          {!! $data['inboxMessage']->appends(Input::except('page'))->render() !!}
                      </div>
                  @else 
