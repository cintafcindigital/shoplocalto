		...
		<div class="pure-g">
			<div class="pure-u-2-7">
				<div class="mr40">
					<nav class="adminAside bills_list">
						...
						<a class="adminAside__item active" href="{{url('bills')}}">
						...
						<a class="adminAside__item " href="{{url('featured')}}">
							<i class="svgIcon svgIcon__receipt adminAside__icon">
		    ...
		    <div class="pure-u-5-7 bills_wrp">
				...
				<div class="adminAlert">
					<p class="adminAlert__title">Check your pending invoices and pay for services on My Health Squad.</p>
					<p> Click into each invoice to make a payment.</p>
				</div>
				<table class="pure-table mb30" cellpadding="0" cellspacing="0" width="100%">
					<thead>
						<tr>
							<td align="right" width="30%">
								<p class="m0 regular">Due date</p>
							</td>
							<td align="right" width="22%">
								<p class="m0 regular">Bill No.</p>
							</td>
							<td align="right" width="22%">
								<p class="m0 regular">Amount</p>
							</td>
							<td align="right">&nbsp;</td>
						</tr>
					</thead>
					<tbody>
						@foreach($data['vendorBill'] as $nm => $bl)
							@php
							$styles = '';
							if(strtotime($bl->due_date) < time()) {
								$styles = 'background:#ff3838';
							}
							@endphp
							@if($nm == 0)
							<tr bgcolor="#FFF6E1">
							...
							<tr bgcolor="#FFFFFF">
							@endif
								<td width="30%" align="right" class="bgColorBorder"> {{ date_format(date_create($bl->due_date),'d/m/Y') }} @if($nm == 0)<span class="fleft tag tag-orange" style="{{ $styles }}"> Due soon </span>@endif </td>
								<td width="22%" align="right" class="mw-font-weight bgColorBorder"> {{ $bl->invoice_number }} </td>
								<td width="22%" align="right" class="bgColorBorder"> C$ {{ $bl->paid_amount }} </td>
								<td align="right" class="bgColorBorder">
									<div class="overflow text-center">
										<div class="right">
											@if($nm == 0)
											<a class="btn-flat red small" href="{{ url('payBy-card').'/'. encrypt($bl->id) }}">Pay</a>
											@else
											<a class="btn-flat red small" href="javascript:void(0)" disabled>Pay</a>
											@endif
										</div>
									</div>
								</td>
				...
				<!-- Paid Bills lists -->
				<button class="app-show-bills btn-outline outline-red mb20 view_paid_btn">View paid invoices</button>
				<div class="app-show-bills-container view_paid_table" style="display:none;">
					<h2 class="adminSubtitle">Paid</h2>
					<table class="pure-table mb20" cellpadding="0" cellspacing="0" width="100%">
						<thead>
							<tr>
								<td align="right" width="30%"><p class="m0 regular">Due date</p></td>
								<td align="right" width="22%"><p class="m0 regular">Bill No.</p></td>
								<td align="right" width="22%"><p class="m0 regular">Amount</p></td>
								<td align="right"><p class="m0 regular">Paid on Date</p></td>
							</tr>
						</thead>
						<tbody class="app-paid-receipts">
							@foreach($data['paidVendorBill'] as $pvb)
							<tr bgcolor="#FFFFFF">
								<td width="30%" align="right" class="bgColorBorder">{{date_format(date_create($pvb->due_date),'d/m/Y')}}<span class="fleft tag tag-green"> Paid </span></td>
								<td width="22%" align="right" class="mw-font-weight bgColorBorder">{{ $pvb->invoice_number }}</td>
								<td width="22%" align="right" class="bgColorBorder">C${{$pvb->paid_amount}}</td>
								<td align="right" class="bgColorBorder">{{date_format(date_create($pvb->paid_date),'M d, Y')}}</td>
							</tr>
							@endforeach
						</tbody>
					</table>
...
$('.view_paid_btn').click(function(){
	$(this).hide();
	$('.view_paid_table').show();
});
</script>
@endsection