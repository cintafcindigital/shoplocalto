@extends('layouts.default')
@section('meta_title',$data['titleData']->meta_title)
@section('meta_keyword',$data['titleData']->meta_keyword)
@section('meta_description',$data['titleData']->meta_description)
@section('content')
@include('includes.menu')

    <section class="section-padding dashboard-wrap dash_main_sect">
     @include('vendor.tools_nav');
        <div class="wrapper">
            <div class="pure-g">
                <div class="pure-u-1-5">
                     @include('includes.messagesidebar')
                </div>

                <div class="pure-u-4-5">

                    @if (\Session::has('message'))
                        <div class="app-hide-alert">
                            <div class="adminAlert adminAlert--success">
                                <p>{!! \Session::get('message') !!}</p>
                            </div>
                        </div>
                    @endif

                    <div id="app-alert-no-leida" class="app-alert adminAlert adminAlert--success" style="display:none;">
                        <p>Lead marked as Unread.</p>
                    </div>

                    <div id="app-alert-leida" class="app-alert adminAlert adminAlert--success" style="display:none;">
                        <p>Lead marked as Read.</p>
                    </div>

                    <div class="pure-g">
                        <div class="pure-u-5-7" style="width:100%;">
                            <div class="mb30 clearfix">
                                <div class="fleft">
                                    <div class="adminFiltersBox">
                                        <div class="app-ui-dropdown adminFiltersBox__select move_select app-va-status-dropdown">

                                            @if($data['message_details']->reply_status == 0)
                                                <span class="app-va-status-selected"><i class="adminBullet adminBullet--orange"></i>Pending</span>
                                            @elseif($data['message_details']->reply_status == 1)
                                                <span class="app-va-status-selected"><i class="adminBullet adminBullet--blue"></i>Replied</span>
                                            @elseif($data['message_details']->reply_status == 2)
                                                <span class="app-va-status-selected"><i class="adminBullet adminBullet--red"></i>Discarded</span>
                                            @elseif($data['message_details']->reply_status == 3)
                                                <span class="app-va-status-selected"><i class="adminBullet adminBullet--green"></i>Booked</span>
                                            @endif
                                            <i class="icon icon-arrow-down"></i>

                                            <ul class="app-ui-dropdown-layer adminFiltersBox__dropdown move_to_list" style="display:none;">

                                                <li id="0_status" class="statusCms @if($data['message_details']->reply_status == 0) dnone app-va-status-hidden @endif" data-status="0">
                                                    <a href="{{ url('messages-status-details') }}/{{ $data['message_details']->id }}/0">
                                                        <i class="adminBullet adminBullet--orange"></i>Pending
                                                    </a>
                                                </li>

                                                <li id="1_status" class="statusCms @if($data['message_details']->reply_status == 1) dnone app-va-status-hidden @endif" data-status="1">
                                                    <a href="{{ url('messages-status-details') }}/{{ $data['message_details']->id }}/1">
                                                        <i class="adminBullet adminBullet--blue"></i>Replied
                                                    </a>
                                                </li>
                                                
                                                <li id="2_status" class="statusCms @if($data['message_details']->reply_status == 2) dnone app-va-status-hidden @endif" data-status="4">
                                                    <a href="{{ url('messages-status-details') }}/{{ $data['message_details']->id }}/2">
                                                        <i class="adminBullet adminBullet--red"></i>Discarded
                                                    </a>
                                                </li>

                                                <li id="3_status" class="statusCms @if($data['message_details']->reply_status == 3) dnone app-va-status-hidden @endif" data-status="3">
                                                    <a href="{{ url('messages-status-details') }}/{{ $data['message_details']->id }}/3">
                                                        <i class="adminBullet adminBullet--green"></i>Booked
                                                    </a>
                                                </li>

                                            </ul>
                                        </div>
                                    </div>
                                    <div class="app-ui-dropdown adminFiltersBox__select mark_select">

                                        <span class="app_combo_leida @if(!$data['message_details']->read_status) dnone  @endif">Read</span>
                                        <span class="app_combo_no_leida @if($data['message_details']->read_status) dnone  @endif">Unread</span>
                                       
                            
                                        <i class="icon icon-arrow-down"></i>
                                        <ul class="app-ui-dropdown-layer adminFiltersBox__dropdown adminFiltersBox__dropdown--noBorder mark_us_list" style="display:none;">
                                            <li class="app_combo_no_leida">
                                                <a href="javascript:;" onclick="statusChangeajax({{ $data['message_details']->id}}, 0)">Unread</a>
                                            </li>
                                            <li class="app_combo_leida dnone">
                                                <a href="javascript:;" onclick="statusChangeajax({{$data['message_details']->id}}, 1)">Read </a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>

                                <div class="fright text-right">

                                    <a class="btnOutline btnOutline--grey ml5" role="button" onclick="leadprint()" style="border: 1px solid #ccc">
                                        <i class="icon icon-print mr5"></i>
                                        <span class="notablet">Print</span>
                                    </a>

                                    <a class="btnOutline btnOutline--grey ml5" data-toggle="modal" href="javascript:;" data-target="#app-va-modal-addnode" style="border: 1px solid #ccc">
                                        <i class="icon icon-note mr5"></i>
                                        <span class="notablet">Add note</span>
                                    </a>

                                    <a class="btnOutline btnOutline--grey ml5" data-toggle="modal" href="javascript:;" data-target="#app-va-modal-sendlead" style="border: 1px solid #ccc">
                                        <i class="icon icon-forward mr5"></i>
                                        <span class="notablet">Forward</span>
                                    </a>

                                </div>
                            </div>

                            <a id="redactar"></a>

                            <div id="app-new-inbox-message-request" class="inbox-message-request inbox-message-request-user" style="padding-right: 0">
                                <div class="pure-g">
                                    <div class="pure-u-1-9">
                                        @if($data['vendorData'][0]['image_data'][0]['image'])
                                            <div class="avatar-vendor">
                                                <img class="avatar-thumb" src="{{url('public/vendors/VENDOR_'.$data['vendorData'][0]['vendor_id'].'/'.$data['vendorData'][0]['image_data'][0]['image'])}}" alt="User Image" width="64px" height="64px" style="height: 60px; width: 60px">
                                            </div>
                                        @else
                                            <div class="avatar-alias size-avatar-medium">
                                                <svg class="avatar-generic" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" preserveAspectRatio="xMidYMin slice">
                                                    <circle fill="#C7C9C0" cx="100" cy="100" r="100"></circle>
                                                    <text transform="translate(100,130)" y="0">
                                                        <tspan font-size="90" class="" fill="rgba(0,0,0,0.3)" text-anchor="middle">{{ ucfirst($data['vendorData'][0]['contact_person'][0]) }}</tspan>
                                                    </text>
                                                </svg>
                                            </div>
                                        @endif
                                    </div>

                                    <div class="pure-u-8-9">
                                        <div class="tools-inbox-message-reply">

                                            <form class="app-vendors-solicitudes-response-form" action="{{ url('/send-reply-message') }}" name="frmToolLayer" id="frmToolLayer" method="post" enctype="multipart/form-data">
                                                {{csrf_field()}}

                                                <div class="box inbox-message-request__requestBox">
                                                    <!--<div class="inbox-message-request__templates app-va-template-selector" style="">
                                                        <div class="app-ui-dropdown">
                                                            <span>Template</span>
                                                            <i class="icon icon-arrow-down"></i>
                                                            <ul class="app-ui-dropdown-layer adminFiltersBox__dropdown app-va-select-template-dropdown" style="display:none;">
                                                                @foreach($data['messageTemplate'] as $smstemplate)
                                                                <li>
                                                                    <a onclick="setMessagetemplate({{ $smstemplate['id'] }});">{{ str_limit($smstemplate['template_name'], $limit = 17, $end = '...') }}</a>
                                                                </li>
                                                                @endforeach
                                                            </ul>
                                                        </div>
                                                    </div>-->

                                                    <input type="hidden" id="is_template" name="is_template" value="">

                                                    <input type="hidden" id="enqury_id" name="enqury_id" value="{{ $data['message_details']->id }}">
                                                    <input type="hidden" id="enq_name" name="enq_name" value="{{ $data['message_details']->name }}">
                                                    <input type="hidden" id="enq_email" name="enq_email" value="{{ $data['message_details']->email }}">

                                                    <div class="hidenfilesarray" style="display: none;"></div>


                                                    <textarea id="app-trumbowyg-editor-va-chat" name="reply_message" class="app-trumbowyg-editor-va-chat trumbowyg-textarea" cols="65" rows="6" placeholder="Write a message" tabindex="-1" style="min-height: 158px; max-height: 408px;width:100%;"></textarea>
                                                    <style>
                                                        div[role="application"] {
                                                            width: 100% !important;
                                                        }
                                                    </style>
                                                            

                                                    <div id="ficheroSubido" class=""></div>


                                                    <div class="app-va-hot-btn-wrapper inbox-message-hot">
                                                        <button class="app-ureq-hot-btn inbox-message-hot__btn" data-message="Unfortunately, we are not provide your requested feature. If you have any flexibility, please let us know. Thank you for your interest!">Unavailable</button>
                                                        <button class="app-ureq-hot-btn inbox-message-hot__btn" data-message="Thank you for your interest in our business! At this point in time, we do not feel our services are the best fit for your needs. We wish you luck on your search for your perfect vendor!">Not interested</button>
                                                    </div>

                                                    <div class="inbox-message-reply-footer">
                                                        <button class="inbox-message-reply-footer__upload icon icon-clip icon-left app-va_solis-upload-file">Attach files</button>

                                                        <input class="dnone app-va-input-upload-file" name="fileupload" id="fileupload" type="file" accept=".jpg, .jpeg, .gif, .png, .doc, .docx, .pdf, .ppt, .pptx, .pps, .xls, .xlsx">

                                                        <!--<span class="app_check_plantilla mr20 color-grey">
                                                            <div class="icheckbox_minimal" style="position: relative;">
                                                                <input type="checkbox" value="yes" name="savetemplate" id="savetemplate" style="position: absolute; top: -20%; left: -20%; display: block; width: 140%; height: 140%; margin: 0px; padding: 0px; background: rgb(255, 255, 255) none repeat scroll 0% 0%; border: 0px none; opacity: 0;">
                                                                <ins class="iCheck-helper" style="position: absolute; top: -20%; left: -20%; display: block; width: 140%; height: 140%; margin: 0px; padding: 0px; background: rgb(255, 255, 255) none repeat scroll 0% 0%; border: 0px none; opacity: 0;"></ins>
                                                            </div>

                                                            <label for="savetemplate">Save as template</label>
                                                        </span>-->

                                                        <input class="btnFlat btnFlat--primary app-va-solic-chat-submit-btn" type="submit" value="Reply">
                                                    </div>
                                                </div>
                                            </form>

                                            @if (\Session::has('reply'))
                                                <div class="app-hide-alert">
                                                    {!! \Session::get('reply') !!}
                                                </div>
                                            @endif

                                            @if ($errors->any())
                                                <div class="alert alert-danger">
                                                    <ul>
                                                        @foreach ($errors->all() as $error)
                                                            <li>{{ $error }}</li>
                                                        @endforeach
                                                    </ul>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            </div>

                            @if(count($data['reply']) > 0)

                                @foreach( $data['reply'] as $smsreply )
                                    @if($smsreply['reply_by'] == $data['vendorData'][0]['vendor_id'] && $smsreply['message_type'] == 'replies')

                                        <div class="app-sol-reply inbox-message-request inbox-message-request-user send-by-vendor" style="padding: 0">
                                            <div class="pure-g">
                                                <div class="pure-u-1-9">
                                                    @if($data['vendorData'][0]['image_data'][0]['image'])
                                                    <div class="avatar-vendor">
                                                        <img class="avatar-thumb" src="{{url('public/vendors/VENDOR_'.$data['vendorData'][0]['vendor_id'].'/'.$data['vendorData'][0]['image_data'][0]['image'])}}" alt="User Image" width="64px" height="64px" style="height: 64px; width: 64px">
                                                    </div>
                                                    @else
                                                    <div class="avatar-alias size-avatar-medium fright">
                                                        <svg class="avatar-generic" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" preserveAspectRatio="xMidYMin slice">
                                                            <circle fill="#C7C9C0" cx="100" cy="100" r="100"></circle>
                                                            <text transform="translate(100,130)" y="0">
                                                                <tspan font-size="90" class="" fill="rgba(0,0,0,0.3)" text-anchor="middle">{{ ucfirst($data['vendorData'][0]['contact_person'][0]) }}</tspan>
                                                            </text>
                                                        </svg>
                                                    </div>
                                                    @endif
                                                </div>
                                                <div class="pure-u-8-9">
                                                    <div class="tools-inbox-message  inbox-message-content-user">
                                                        {!! $smsreply['message'] !!}

                                                        @if(count($smsreply['reply_images']) > 0)
                                                            @foreach( $smsreply['reply_images'] as $replyimagesv)
                                                                <div class="box-sol-reply-links mb20">
                                                                    <a style="background-color: #fff" class="inbox-message-link icon icon-clip icon-left" target="blank" href="{{ url('replyimages') }}/{{ $replyimagesv['image_name'] }}">{{ $replyimagesv['original_name'] }}</a>
                                                                </div>
                                                            @endforeach
                                                        @endif
                                                        
                                                        <time>
                                                            <span class="adminConversation__status">
                                                                @if($smsreply['is_read'])
                                                                    <i class="icon icon-double-check icon-left"></i>
                                                                    Read by client
                                                                @else
                                                                    <i class="icon icon-single-check icon-left"></i>
                                                                    Delivered to client
                                                                @endif
                                                            </span>on {{ date('d/M/Y', strtotime($smsreply['created_at'])) }} at {{ date('H:i', strtotime($smsreply['created_at'])) }}
                                                        </time>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        @elseif($smsreply['reply_by'] == $data['vendorData'][0]['vendor_id'] && $smsreply['message_type'] == 'notes')
                                            <div class="app-sol-reply inbox-message-request inbox-message-request-note" >
                                                <div class="pure-g">
                                                    <div class="pure-u-1-9"></div>
                                                    <div class="pure-u-8-9">
                                                        <div class="tools-inbox-message  inbox-message-content-note" style="background-color: #fffaed">
                                                            <b>Note : </b> &nbsp; {!! $smsreply['message'] !!}
                                                            <time>
                                                                on {{ date('d/M/Y', strtotime($smsreply['created_at'])) }} at {{ date('H:i', strtotime($smsreply['created_at'])) }}
                                                            </time>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>

                                        @elseif($smsreply['reply_by'] == $smsreply['user_id'] && $smsreply['message_type'] == 'replies')

                                            <div class="app-sol-reply inbox-message-request inbox-message-request-vendor received-by-user">
                                                <div class="pure-g">
                                                    <div class="pure-u-8-9">
                                                        <div class="tools-inbox-message inbox-message-content-vendor  box">
                                                            <div class="tools-inbox-message__header">
                                                                <p class="strong">{{ ucfirst($smsreply['user']['name']) }}</p>
                                                            </div>

                                                            {!! $smsreply['message'] !!} 

                                                             @if(count($smsreply['reply_images']) > 0)
                                                                @foreach( $smsreply['reply_images'] as $replyimagesv)
                                                                    <div class="box-sol-reply-links mb20">
                                                                        <a style="background-color: #fff" class="inbox-message-link icon icon-clip icon-left" target="blank" href="{{ url('replyimages') }}/{{ $replyimagesv['image_name'] }}">{{ $replyimagesv['original_name'] }}</a>
                                                                    </div>
                                                                @endforeach
                                                            @endif

                                                            <time>
                                                                on {{ date('d/M/Y', strtotime($smsreply['created_at'])) }} at {{ date('H:i', strtotime($smsreply['created_at'])) }}
                                                            </time>
                                                        </div>
                                                    </div>
                                                    <div class="pure-u-1-9">
                                                        @if($smsreply['user']['profile_image'])
                                                        <figure class="fright">
                                                            <img class="avatar-thumb" src="{{ url('storage/USER_').$smsreply['user']['id'] }}/{{ $smsreply['user']['profile_image'] }}" alt="User Image" width="64px" height="64px" style="height: 64px; width: 64px">
                                                        </figure>
                                                        @else
                                                        <div class="avatar-alias size-avatar-medium">
                                                            <svg class="avatar-generic" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" preserveAspectRatio="xMidYMin slice">
                                                                <circle fill="#C7C9C0" cx="100" cy="100" r="100"></circle>
                                                                <text transform="translate(100,130)" y="0">
                                                                    <tspan font-size="90" class="" fill="rgba(0,0,0,0.3)" text-anchor="middle">{{ ucfirst($smsreply['user']['name'][0]) }}</tspan>
                                                                </text>
                                                            </svg>
                                                        </div>
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>

                                        @elseif($smsreply['reply_by'] == $smsreply['user_id'] && $smsreply['message_type'] == 'notes')

                                            <div class="app-sol-reply inbox-message-request inbox-message-request-note" >
                                                <div class="pure-g">
                                                    
                                                    <div class="pure-u-8-9">
                                                        <div class="tools-inbox-message  inbox-message-content-note" style="background-color: #fffaed">
                                                            <div class="tools-inbox-message__header">
                                                                <p class="strong">{{ ucfirst($smsreply['user']['name']) }}</p>
                                                            </div>
                                                            <b>Note : </b> &nbsp; {!! $smsreply['message'] !!}
                                                            <time>
                                                                on {{ date('d/M/Y', strtotime($smsreply['created_at'])) }} at {{ date('H:i', strtotime($smsreply['created_at'])) }}
                                                            </time>
                                                        </div>
                                                    </div>
                                                    <div class="pure-u-1-9"></div>
                                                </div>
                                            </div>
                                        @endif

                                @endforeach

                            @endif


                            @if(isset($data['message_details']))
                                <div class="inbox-message-request-sent">
                                    <span class="inbox-message-request-sent__title">Received on {{ date('d/M/Y', strtotime($data['message_details']->created_at)) }} at {{ date('H:i', strtotime($data['message_details']->created_at)) }} </span>
                                </div>

                                <div class="inbox-message-request inbox-message-request-vendor">
                                    <div class="pure-g">
                                        <div class="pure-u-8-9">
                                            <div class="tools-inbox-message inbox-message-content-vendor box">
                                                <p class="strong"><b>{{ $data['message_details']->name }}</b></p>

                                                <div class="inbox-message-request-details">
                                                    <span>
                                                        <i class="inbox-vendor-profileList__icon icon-vendor icon-vendor-phone"></i> Phone number: {{ $data['message_details']->phone }}
                                                    </span>

                                                    <span><i class="inbox-vendor-profileList__icon icon-vendor icon-vendor-email"></i> Email:{{ $data['message_details']->email }} </span>
                                                </div>

                                                <hr class="adminConversation__separator">
                                                <p class="adminConversation__comment">
                                                    {!! $data['message_details']->comment !!}
                                                </p>
                                            </div>
                                        </div>

                                        <div class="pure-u-1-9">
                                            @if(@$data['message_details']->user->profile_image)
                                            <figure class="fright">
                                                <img class="avatar-thumb" src="{{ url('storage/USER_').$data['message_details']->user->id }}/{{ $data['message_details']->user->profile_image }}" alt="User Image" width="64px" height="64px">
                                            </figure>
                                            @else
                                            <div class="avatar-alias size-avatar-medium fright">
                                                <svg class="avatar-generic" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" preserveAspectRatio="xMidYMin slice">
                                                    <circle fill="#C7C9C0" cx="100" cy="100" r="100"></circle>
                                                    <text transform="translate(100,130)" y="0">
                                                        <tspan font-size="90" class="" fill="rgba(0,0,0,0.3)" text-anchor="middle">{{ ucfirst(@$data['message_details']->name[0]) }}</tspan>
                                                    </text>
                                                </svg>
                                            </div>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            @endif

                        </div>

                        <div class="pure-u-2-7" style="display:none;">
                            <div class="ml30">
                                <div class="box">
                                    <div class="inbox-vendor-profile">
                                        @if($data['message_details'])
                                            <div class="inbox-vendor-profile__content">
                                                <div class="text-center">
                                                    @if(@$data['message_details']->user->profile_image)
                                                        <figure class="avatar-center">
                                                            <img class="avatar-thumb" src="{{ url('storage/USER_').$data['message_details']->user->id }}/{{ $data['message_details']->user->profile_image }}" alt="Profile" style="width: 95px; height: 95px;">
                                                        </figure>
                                                     @else
                                                        <div class="avatar-alias size-avatar-xmedium avatar-center">
                                                            <svg class="avatar-generic" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" preserveAspectRatio="xMidYMin slice" style="height: 98px">
                                                                <circle fill="#C7C9C0" cx="100" cy="100" r="100"></circle>
                                                                <text transform="translate(100,130)" y="0">
                                                                    <tspan font-size="90" class="" fill="rgba(0,0,0,0.3)" text-anchor="middle">{{ ucfirst(@$data['message_details']->user->name[0]) }}</tspan>
                                                                </text>
                                                            </svg>
                                                        </div>
                                                    @endif
                                                    <span class="inbox-vendor-profile-title mt10">{{ @$data['message_details']->user->name }}</span>Searching : {{ $data['vendorData'][0]['category_data']['title'] }} in {{ $data['vendorData'][0]['company_data']['province'] }} <hr>
                                                </div>
                                                <div class="inbox-vendor-profileList">
                                                    <div class="inbox-vendor-profileList__item">
                                                        <span class="inbox-vendor-profileList__icon icon icon-form-cal"></span>
                                                        <span class="inbox-vendor-profile__mail" style="margin-left: 10px">
                                                            Event: {{ date('d/m/Y', strtotime(@$data['message_details']->user->event_date)) }}
                                                        </span>                                     
                                                    </div>

                                                    <div class="inbox-vendor-profileList__item">
                                                        <span class="inbox-vendor-profileList__icon icon icon-form-mobile"></span>
                                                        <span class="inbox-vendor-profile__mail" style="margin-left: 10px">
                                                            {{ @$data['message_details']->user->phone }}
                                                        </span>
                                                    </div>
                                                    
                                                    <div role="button">
                                                        <div class="inbox-vendor-profileList__item">
                                                            <span class="inbox-vendor-profileList__icon icon icon-form-mail"></span>
                                                            <span class="inbox-vendor-profile__mail" style="margin-left: 10px">
                                                                {{ @$data['message_details']->user->email }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                     
                                                </div>
                                            </div>
                                        @endif
                                    </div>

                                    <div class="vendors-item-content app-tools-vendors-modif" data-id-empresa="17879" data-id-escaparate="">

                                        <div class="app-va-change-status vendorsItemStatus__item" enq-id="{{ $data['message_details']->id }}" data-status-old="{{ $data['message_details']->reply_status }}" onclick="changeStatusforBooking(this)">
                                            <i class="svgIcon svgIcon__checkCircle app-va-change-status-icon vendorsItemStatus__icon vendorsItemStatus__icon--check @if($data['message_details']->reply_status == 3) active @endif">
                                                <svg viewBox="0 0 92 92"><path d="M46 0C20.6 0 0 20.6 0 46s20.6 46 46 46 46-20.6 46-46S71.4 0 46 0zm20.1 36.3L45 59.7c-.5.6-1.3 1-2.1 1h-.1c-.8 0-1.5-.3-2.1-.9L26 45.3c-1.2-1.2-1.2-3.1 0-4.2 1.2-1.2 3.1-1.2 4.2 0l12.4 12.3 19.1-21c1.1-1.2 3-1.3 4.2-.2 1.3 1 1.4 2.9.2 4.1z"></path></svg>
                                            </i>
                                            <span class="vendorsItemStatus__label">Booked</span>
                                        </div>

                                    </div>
                                </div>
                            </div>

                            <div class="box p20 text-center ml30">
                                <i class="icon-vendor icon-vendor-info-circle block mb15"></i>
                                <strong>Get more clients:</strong><br> Reply to new leads and messages within 24 hours
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div id="app-vendor-availability-dialog" data-target="#app-vendor-availability-dialog" class="modal"></div>
</section>

@include('vendor/singlemodel_popup.messagesingle-popup');
@include('vendor.ExportLeads.exportleads_popup')
@include('includes.footer')

<script src="https://cdn.tiny.cloud/1/08cq8y02kwr759p88hfbo9ym579ceup2a7x2vebx42n6ceee/tinymce/5/tinymce.min.js"></script>
<script>
    tinymce.init({
        selector:'#app-trumbowyg-editor-va-chat',
        width: 570,
        height: 300
    });

    $(document).ready(function() {
        // Hide status after status change
            setTimeout(function() {
                $('.app-hide-alert').slideUp();
            }, 4000);

        // file upload click
            $('.inbox-message-reply-footer__upload').on('click', function() {
                $('#fileupload').trigger('click');
                return false;
            });

        // Template checkbox click and hover
            $('.app_check_plantilla').hover( function() {
                $(this).find('.icheckbox_minimal').addClass('hover');
                $(this).find('label').addClass('hover');
            }, function() {
                $(this).find('.icheckbox_minimal').removeClass('hover');
                $(this).find('label').removeClass('hover');
            });

            $('.app_check_plantilla').on('click', function() {
                
                if(!$('#savetemplate').is(":checked")) {
                    $(this).find('.icheckbox_minimal').addClass('checked');
                    $('#savetemplate').prop('checked', true);
                } else {
                    $(this).find('.icheckbox_minimal').removeClass('checked');
                    $('#savetemplate').prop('checked', false);
                }
            });

        //default message
            $('.inbox-message-hot__btn').on('click', function() {
                var messagesDefault = $(this).attr('data-message');
                //alert(messagesDefault);
                tinymce.get("app-trumbowyg-editor-va-chat").setContent(messagesDefault);
                $('.app-va-hot-btn-wrapper').hide();
                return false;
            });

        // upload file ajax
            $("#fileupload").change(function(e) {
                //Here is where you will make your AJAX call
                var fileobj = e.target.files[0];
                var formData = new FormData();
                formData.append("fileobj", fileobj);

                $.ajax({
                    type:'POST',
                    url: "{{ url('reply-fileupload') }}",
                    data:formData,
                    cache:false,
                    contentType: false,
                    processData: false,
                    success:function(data){
                        var data = JSON.parse(data);
                        if(data.html != 'yes') {
                            $('.hidenfilesarray').append('<input id="'+data.id+'" type="hidden" name="inputFiles[]" value="'+data.fileid+'" />');
                            $('#ficheroSubido').addClass('app-inbox-upload-attachments');
                            $('#ficheroSubido').append(data.html);
                        }
                    },
                    error: function(data){
                        console.log("error");
                        console.log(data);
                    }
                });
            });

        // Remove image from upload
            $('body').on('click', '.inbox-message-link__remove', function() {
                var removeid = $(this).attr('data-remove');
               // alert(removeid);
                $('body .hidenfilesarray').find('#'+removeid).remove();
                $(this).parents('.inbox-message-link').remove();
            });

        // Template dropdow
            $('.inbox-message-request__templates').on('click', function() {
                $(this).find('.app-va-select-template-dropdown').toggle();
            });

    });
    
    // Message status chage Ajax
        function statusChangeajax(eqid, status) {
            $.ajax({
                type:'get',
                url: '{{ url("messages-details-status") }}/'+eqid+'/'+status,
                success: function(responce) {
                    if(responce == 1) {
                        $('#app-alert-no-leida').hide();
                        $('#app-alert-leida').show();
                        $('.app_combo_leida').removeClass('dnone');
                        $('.app_combo_no_leida').addClass('dnone');

                        $('.mark_us_list .app_combo_leida').addClass('dnone');
                        $('.mark_us_list .app_combo_no_leida').removeClass('dnone');

                    }
                    if(responce == 0) {
                        $('#app-alert-no-leida').show();
                        $('#app-alert-leida').hide();
                        $('.app_combo_leida').addClass('dnone');
                        $('.app_combo_no_leida').removeClass('dnone');

                        $('.mark_us_list .app_combo_leida').removeClass('dnone');
                        $('.mark_us_list .app_combo_no_leida').addClass('dnone');
                    }
                }
            });
        }

    // Set message template 

        function setMessagetemplate(templateid) {
            //alert(templateid);
            $.ajax({
                'type': "get",
                'url': '{{ url("set-message-template") }}/'+templateid,
                success: function(responce) {
                    var responce = JSON.parse(responce);
                    $('#is_template').val('yes');
                    tinymce.get("app-trumbowyg-editor-va-chat").setContent(responce.messageContent);
                    //console.log(responce);
                    if(responce.imageHtml) {
                        $('#ficheroSubido').addClass('app-inbox-upload-attachments');
                        $('#ficheroSubido').html(responce.imageHtml);
                    }else {
                        $('#ficheroSubido').html('');
                    }
                    if(responce.inputHidden) {
                        $('.hidenfilesarray').html(responce.inputHidden);
                    }
                    else {
                         $('.hidenfilesarray').html('');
                    }
                }
            })
        }

    // Save Note

        function saveNote() {
           var x = $('#message_note').val();
           if(x.trim().length <= 0) {
                alert('The comment must contain at least 5 characters.');
                return false;
           }
        }

     // Save Note

        function resendMessage() {
           var x = $('#resend_mail').val();
           if(x.trim().length <= 0) {
                alert('Please Enter Valid Email ID');
                return false;
           }
        }

    // Lead print

        function leadprint(leadid) {
            window.open('{{ url("/print-lead") }}/{{ $data["message_details"]->id }}','popUpWindow','height=800,width=800,left=200,top=50,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes');
        }

    // Booking status change on Message single page

        function changeStatusforBooking(cvthis) {
            console.log(cvthis);
            var leadid = cvthis.getAttribute('enq-id');
            var cleadstatus = cvthis.getAttribute('data-status-old');
            if(cleadstatus == 3) {
                cleadstatus = 0;
            }
            $.ajax({
                'type': 'post',
                'url': '{{ url("booking-status") }}',
                'data': {'leadid':leadid, 'cleadstatus':cleadstatus},
                success: function(responce) {
                    var responce = JSON.parse(responce);
                    if(responce.active == 'yes') {
                        $('.vendorsItemStatus__icon').addClass('active');
                    }else {
                        $('.vendorsItemStatus__icon').removeClass('active');
                    }

                    var statusHtml = '';

                    if(responce.leadstatus == 0) {
                        statusHtml = '<i class="adminBullet adminBullet--orange"></i>Pending';
                    }
                    if(responce.leadstatus == 1) {
                        statusHtml = '<i class="adminBullet adminBullet--blue"></i>Replied';
                    }
                    if(responce.leadstatus == 2) {
                        statusHtml = '<i class="adminBullet adminBullet--red"></i>Discarded';
                    }
                    if(responce.leadstatus == 3) {
                        statusHtml = '<i class="adminBullet adminBullet--green"></i>Booked';
                    }
                // For page status
                    $('.statusCms').removeClass('dnone');
                    $('#'+responce.leadstatus+'_status').addClass('dnone');
                    $('.app-va-status-selected').html(statusHtml);

                // For sidebar status
                    $('.app-va-folders-side-status').removeClass('active');
                    $('#'+responce.leadstatus+'_sidebarstatus').addClass('active');

                // For sidebarCounter
                    // For update
                        var cu = $('#'+responce.leadstatus+'_sidebarstatus').find('.app-va-folders--counter').text();
                        cu = parseInt(cu) + parseInt(1);
                        $('#'+responce.leadstatus+'_sidebarstatus').find('.app-va-folders--counter').text(cu);
                    // For Lowdown
                        if(cleadstatus != responce.leadstatus) {
                            var cd = $('#'+cleadstatus+'_sidebarstatus').find('.app-va-folders--counter').text();
                            cd = parseInt(cd) - parseInt(1);
                            $('#'+cleadstatus+'_sidebarstatus').find('.app-va-folders--counter').text(cd);
                        }else {
                            var cd = $('#3_sidebarstatus').find('.app-va-folders--counter').text();
                            cd = parseInt(cd) - parseInt(1);
                            $('#3_sidebarstatus').find('.app-va-folders--counter').text(cd);
                        }
                }
            })
        }

</script>
@endsection