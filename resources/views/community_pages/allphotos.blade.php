            ...
            <div class="group-info-wr">
                <div class="group-inner-wr">
                    <div class="no-margin col-xs-12 col-sm-12 col-md-9 cm-gp-left">
                            ...
                            <div class="add-dis-checker row">
                               <div class="col-xs-12 col-sm-12 col-md-6">
                                ...
                                <div class="col-xs-12 col-sm-12 col-md-6 text-right">
                                     <a class="add-dis-video-group" onclick="addgroupimages({{ $communityData['id'] }})" href="javascript:;">Add Photos</a>
                                </div>
                            </div>
                       ...
                       @if( count($communityData['groupImages']) > 0 )
                        <div class="discussion-header-title border-bottom">
                            <h2>Photos</h2>
                        </div>
                        ...
                        <div class="cls-group-photos-wr">
                            <div class="cls-group-photos-inner-wr">
                                <div class="grid">
                                @foreach($communityData['groupImages'] as $groupImagesdata)
                                    <div class="grid-item pure-u-1-3">
                                        <div class="box">
                                            <figure>
                                                <a class="photo-zoom app-com-media-item-layer" data-fancybox="gallery" href="{{ url('/public/discussion_group_images') }}/{{ $groupImagesdata['community_image'] }}">
                                                    <img alt="" src="{{ url('/public/discussion_group_images') }}/{{ $groupImagesdata['community_image'] }}">
                                                </a>
                                            <figcaption class="discussion-picture-meta">
                                                <div class="pure-g">
                                                    <div class="pure-u-1-6">
                                                        @if($groupImagesdata['user']['profile_image'] != '')
                                                            <a  class="app-community-profile-layer avatar" href="javascript:;">
                                                                <figure>
                                                                    <img class="avatar-thumb" src="{{ url('storage/USER_').$groupImagesdata['user']['id'].'/'.$groupImagesdata['user']['profile_image'] }}" alt="Seleena">
                                                                </figure>
                                                            </a>
                                                        @else
                                                            <div class="avatar-alias size-avatar-medium app-discussion-comment-avatar">
                                                                <svg class="avatar-generic" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" preserveAspectRatio="xMidYMin slice">
                                                                    ...
                                                                    <text transform="translate(100,130)" y="0">
                                                                        <tspan font-size="90" class="" fill="rgba(0,0,0,0.3)" text-anchor="middle">{{ ucfirst($groupImagesdata['user']['name'][0]) }}</tspan>
                                                                    </text>
                                                                </svg>
                                                            </div>
                                                    ...
                                                    <div class="pure-u-5-6 pl10">
                                                        <a class="discussion-picture-title app-com-media-item-layer" href="{{ url('/community/forums') }}/{{ $groupImagesdata['discussion']['discussion_slug'] }}">
                                                           {{ $groupImagesdata['discussion']['discussion_title'] }}
                                                        </a>
                                                        <span class="discussion-picture-author">By {{ $groupImagesdata['user']['name'] }}</span>
                                                    </div>
                                                </div>
                                            </figcaption>
                                        </figure>
...
@include('includes.add_photo_popup')
...
<script type="text/javascript">
    ...
    function addgroupimages(group_id) {
        $('#imageGroupid').val(group_id);
    ...
    $(document).ready(function() {
        ...
        $('body').on('click', '.group-image-save', function() {
            var photeCheck = document.getElementById("groupImageid").files.length;

            var groupid = $('#imageGroupid').val();
            var imageuploadData = new FormData();

            imageuploadData.append( 'groupid', groupid);
            $.each($("#groupImageid"), function(i, obj) {       // Upload multiple by $each loop
                    $.each(obj.files,function(j, file){
                        imageuploadData.append('groupImage['+j+']', file);
                    })
            });
            ...
            if( photeCheck != 0 ){
                $.ajax({
                    type:'POST',
                    url: '{{ url("community/post-group-images") }}',
