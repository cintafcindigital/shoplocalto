    ...
    <div class="pure-g">
                    ...
                    <div class="app-tools-budget-rows subtitle">
                      @if(isset($data['child_cats']) && !empty($data['child_cats']))
                       ...
                       @endphp
                        ...
                        <div style="height:54px;" class="budget-categories-item icon icon-arrow-right app-tools-budget-row app-budget-scroll-top @if($data['cat_id'] == $cat['cat_id']) current @endif" data-category-id="{{$cat['cat_id']}}" >
        ...
        <div class="pure-u-2-3 budget-detail">
           @if(Session::has('message'))
             {!!Session::get('message')!!}
           @endif
           <div class="app-category-budget app-budget-aside budget-categDetail" data-section-active="categDetail"><div class="app-category-budget app-budget-aside box">
            <div class="budget-category-header text-center">
                <a href="{{url('tools/budget')}}"><span class="app-close-category-detail icon icon-close pointer"></span></a>
                <p class="budget-category-header-title app-category-title text-center">
                    <span class="app-category-title-text">{{$data['currentCat'][0]['title'] ?? 'Category'}}</span>
                            </p>
                <p class="budget-category-header-available app-budget-detail-status-text mb10">
                    <span class="ml10">Estimated budget: <strong class="app-available-category-estimado">
                            C$ <span class="app-budget-estimated-cost estimated-cost-new">{{$data['total_estimate'] ?? 0}}</span>
                    </strong>
                    </span>
                    <span class="ml10">Final cost: <strong class="app-final-category-total color-green">
                            C$  <span class="app-budget-final-cost">{{$data['total_final_cost'] ?? 0}}</span>
                    </strong>
                    </span>
                    <!-- <span class="app-delete-category ml10 color-grey pointer icon-tools icon-tools-trash-grey icon-left tools-toggle-action" data-category-id="215">
                        Remove            </span> -->
                </p>
            </div>
    <div class="budget-category-detail">
        <div class="pure-g budget-category-detail-header">
            <div class="pure-u-1-4 budget-spending-item-cell">Concept</div>
            <div class="pure-u-5-24 text-right budget-spending-item-cell">Estimated budget</div>
            <div class="pure-u-5-24 text-right budget-spending-item-cell">Final cost</div>
            <div class="pure-u-5-24 text-right budget-spending-item-cell">Paid</div>
            <div class="pure-u-1-8 text-right budget-spending-item-cell"></div>
        </div>
        @if(isset($data['budgets']) && !empty($data['budgets']))
          @foreach($data['budgets'] as $budget)
            <div class="app-budget-parent-spending-row">
                <div class="budget-spending-item app-spending-row pure-g" data-category-id="216" data-parent-id="215">
                    <div class="pure-u-1-4 budget-spending-item-cell">
                        <div class="input-group-line subtitle">
                            <input class="app-new-spending" data-budget-id="{{$budget['id']}}" data-field="concept" type="text" name="spendingName" value="{{$budget['concept']}}" data-val="" placeholder="Expense name should have at least three characters." onblur="Frontend.updateBudgetData(this)">
                        </div>
                    </div>
                    <div class="pure-u-5-24 budget-spending-item-cell budget-category-detail-cost text-right">
                        <div class="input-group-line input-group-line-currency">
                                <span class="currency">C$</span>
                            <input class="app-new-spending" data-budget-id="{{$budget['id']}}" data-field="estimate_budget" type="text" name="estimated" value="{{number_format($budget['estimate_budget'])}}" data-val="{{number_format($budget['estimate_budget'])}}" data-msgerror="The price must be greater than 0 dollars" onblur="Frontend.updateBudgetData(this)">
                        </div>
                    </div>
                    <div class="pure-u-5-24 budget-spending-item-cell budget-category-detail-cost text-right">
                        <div class="input-group-line input-group-line-currency">
                                <span class="currency">C$</span>
                            <input class="app-new-spending" data-budget-id="{{$budget['id']}}" data-field="final_cost" type="text" name="final" value="{{number_format($budget['final_cost'])}}" data-val="{{number_format($budget['final_cost'])}}" data-msgerror="The price must be greater than 0 dollars" onblur="Frontend.updateBudgetData(this)">
                        </div>
                    </div>
                    <div class="pure-u-5-24 budget-spending-item-cell text-right">
                        <span class="app-spending-payment-total budget-spending-payment app-payment-layer" data-payments-id="216" data-totalpaid="{{number_format($budget['paid'])}}" data-budget-id="{{$budget['id']}}" onclick="Frontend.addBudgetPayment(this)">
                                <span class="currency">C$</span>
                            <span class="app-payment-value">{{$budget['paid']}}</span>
                        </span>
                    </div>
                    <div class="pure-u-1-8 budget-spending-item-cell app-note-row text-right">
                       <span class="app-delete-category ml10 color-grey pointer icon-tools icon-tools-trash-grey tools-toggle-action" data-category-id="{{$budget['id']}}" onclick="Frontend.deleteBudgetData(this)"></span>
                    </div>
                </div>
                <div class="app-show-payments-details" data-payments-details-id="216"></div>
            </div>
            @endforeach
        @endif
        <div class="budget-spending-item-cell">
            <span class="icon-tools icon-left icon-tools-plus-circle-outline mr10 link pointer app-add-spending" onclick="Frontend.showAddBudgetPopup(this)"  data-parent-id="{{$data['currentCat'][0]['cat_id']}}"> Add new expense</span>
        </div>

        <div class="budget-category-detail-footer bg pure-g">
            <div class="pure-u-1-4 budget-spending-item-cell">
            ...
            <div class="pure-u-5-24 budget-spending-item-cell budget-category-detail-cost text-right">
                <span class="strong app-estimated-category-total">
                    C$  <span class="app-budget-estimated-cost estimated-cost-new">{{$data['total_estimate'] ?? 0}}</span>
                   <input type="hidden" id="total_estimate_value" value="{{$data['total_estimate'] ?? 0}}">
                </span>
            </div>
            <div class="pure-u-5-24 budget-spending-item-cell budget-category-detail-cost text-right">
                <span class="strong app-final-category-total color-green">
                    C$  <span class="app-budget-final-cost">{{$data['total_final_cost'] ?? 0}}</span>
                     <input type="hidden" id="total_final_value" value="{{$data['total_final_cost'] ?? 0}}">
                
                </span>
            </div>
            <div class="pure-u-5-24 budget-spending-item-cell text-right">
                <span class="strong mr5 app-available-category-total-payment">
                    C$                    <span class="app-available-category-total-payment-value">{{$data['total_paid'] ?? 0}}</span>
                                    </span>
                  </div>
        </div>
    </div>
